...
export const _createOrg = (organization) => ({ type: CREATE_ORGANIZATION, organization });
export const _removeOrg = (organization) => ({ type: REMOVE_ORGANIZATION, organization });
export const _updateOrg = (organization) => ({ type: UPDATE_ORGANIZATION, organization });
...
export const createOrganization = (org) => {
  return async (dispatch) => {
    const res = await Axios.post('https://foodpiles.herokuapp.com/api/organizations', org);
...
export const removeOrganization = (orgId) => {
  return async (dispatch) => {
    const res = await Axios.delete(`https://foodpiles.herokuapp.com/api/organizations/${orgId}`)
...
export const updateOrganization = (org) => {
  return async (dispatch) => {
    const res = await Axios.put(`https://foodpiles.herokuapp.com/api/organizations/${org.id}`, org);
...
export default function (state = [], action) {
  switch (action.type) {
    ...
    case UPDATE_ORGANIZATION:
      return state.map(org => org.id === action.organization.id ? action.organization : org);
    default:
      return state;
  }
}
